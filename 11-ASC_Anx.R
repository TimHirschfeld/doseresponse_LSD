...
data <- read_excel("11-ASC_Anx.xlsx", col_names = TRUE, col_types = "guess", na = "NULL")

##############################################################################
# Individuelle Regressionsanalyse  #

#defining variables as numeric
Scale_std <- as.numeric(data$Anxiety_std)                     # standard deviation
effect_sizes <- as.numeric(data$Anxiety_mean)                 # mean
...
tiff(filename = "11-ASC_Anx.tiff", units="cm", width=14.55, height=11.684, res=600)
