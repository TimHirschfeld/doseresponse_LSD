...
data <- read_excel("11-ASC_Cha.xlsx", col_names = TRUE, col_types = "guess", na = "NULL")

##############################################################################
# Individuelle Regressionsanalyse  #

#defining variables as numeric
Scale_std <- as.numeric(data$Changed_meaning_of_percepts_std)        # standard deviation
effect_sizes <- as.numeric(data$Changed_meaning_of_percepts_mean)    # mean
...
#defining colors; number corresponds to the number of effect sizes (different doses) for each study) incl. PMID
colors <-   c(rep("#6a3d9a", 1),       #27714429
              rep("#cab2d6", 1),       #34613430
              rep("#b2df8a", 1),       #25575620
              rep("#a6cee3", 1),       #31331617
              rep("black", 1),         #28132813
              rep("#fb9a99", 1),       #26847689
              rep("#ff7f00", 1),       #34253268
              rep("#33a02c", 1),       #31733631
              rep("#e31a1c", 4),       #33059356
              rep("#fdbf6f", 1),       #29167644
              rep("#1f78b4", 2),       #35217796
              rep("#b15928", 4)        #35106880      
)

tiff(filename = "11-ASC_Cha.tiff", units="cm", width=14.55, height=11.684, res=600)
