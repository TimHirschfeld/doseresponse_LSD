...
data <- read_excel("11-ASC_Spi.xlsx", col_names = TRUE, col_types = "guess", na = "NULL")

##############################################################################
# Individuelle Regressionsanalyse  #

#defining variables as numeric
Scale_std <- as.numeric(data$Spiritual_experience_std)        # standard deviation
effect_sizes <- as.numeric(data$Spiritual_experience_mean)    # mean
...
colors <-   c(rep("#6a3d9a", 1),       #27714429 PMID
...
tiff(filename = "11-ASC_Spi.tiff", units="cm", width=14.55, height=11.684, res=600)
