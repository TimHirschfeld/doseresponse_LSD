...
data <- read_excel("11-ASC_Imp.xlsx", col_names = TRUE, col_types = "guess", na = "NULL")

##############################################################################
# Individuelle Regressionsanalyse  #

#defining variables as numeric
Scale_std <- as.numeric(data$Impaired_control_and_cognition_std)        # standard deviation
effect_sizes <- as.numeric(data$Impaired_control_and_cognition_mean)    # mean
...
tiff(filename = "11-ASC_Imp.tiff", units="cm", width=14.55, height=11.684, res=600)
