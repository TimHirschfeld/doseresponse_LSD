...
data <- read_excel("MEQ30_Pos.xlsx", col_names = TRUE, col_types = "guess", na = "NULL")

##############################################################################
# Individuelle Regressionsanalyse  #

#defining variables as numeric
Scale_std <- as.numeric(data$Positive_mood_std)                # standard deviation
effect_sizes <- as.numeric(data$Positive_mood_mean)            # mean
...
tiff(filename = "MEQ30_Pos.tiff", units="cm", width=14.55, height=11.684, res=600)
