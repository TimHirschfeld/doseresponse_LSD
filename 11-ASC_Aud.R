...
data <- read_excel("11-ASC_Aud.xlsx", col_names = TRUE, col_types = "guess", na = "NULL")

##############################################################################
# Individuelle Regressionsanalyse  #

#defining variables as numeric
Scale_std <- as.numeric(data$Audio_visual_synesthesia_std)        # standard deviation
effect_sizes <- as.numeric(data$Audio_visual_synesthesia_mean)    # mean
...
#defining colors; number corresponds to the number of effect sizes (different doses) for each study) inkl. PMID
...
tiff(filename = "11-ASC_Aud.tiff", units="cm", width=14.55, height=11.684, res=600)
