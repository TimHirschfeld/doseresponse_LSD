...
data <- read_excel("5D-ASC_Vig.xlsx", col_names = TRUE, col_types = "guess", na = "NULL")

##############################################################################
# Individuelle Regressionsanalyse  #

#defining variables as numeric
Scale_std <- as.numeric(data$Vigilance_Reduction_std)         # standard deviation
effect_sizes <- as.numeric(data$Vigilance_Reduction_mean)     # mean
...
tiff(filename = "5D-ASC_Vig.tiff", units="cm", width=14.55, height=11.684, res=600)
